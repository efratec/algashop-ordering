...
public class ResilientProductCatalogAPIClient {
    ...
    private RuntimeException translateException(RestClientException error) {
        if (error.getCause() instanceof SocketTimeoutException
            ...
            return new GatewayTimeoutException("Product Catalog API Timeout", error);
        }

        if (error instanceof HttpClientErrorException) {
            return new BadGatewayException.ClientErrorException("Product Catalog API Bad Gateway", error);
        }

        if (error instanceof HttpServerErrorException) {
            return new BadGatewayException.ServerErrorException("Product Catalog API Bad Gateway", error);
        }

        return new BadGatewayException("Product Catalog API Bad Gateway", error);
    }
