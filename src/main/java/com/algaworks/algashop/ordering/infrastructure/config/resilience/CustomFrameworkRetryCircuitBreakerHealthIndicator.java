...
import static com.algaworks.algashop.ordering.infrastructure.config.resilience.SpringCircuitBreakerConfig.RAPIDEX_API_CB;
...
public class CustomFrameworkRetryCircuitBreakerHealthIndicator implements HealthIndicator {
    ...
    public CustomFrameworkRetryCircuitBreakerHealthIndicator(CircuitBreakerFactory circuitBreakerFactory) {
        circuitBreakers.add((FrameworkRetryCircuitBreaker) circuitBreakerFactory.create(PRODUCT_CATALOG_CB));
        circuitBreakers.add((FrameworkRetryCircuitBreaker) circuitBreakerFactory.create(RAPIDEX_API_CB));
    }
